...
export default function PosturalIssueDetail({ issue, onClose }) {
  ...
  const header = (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Tab Selection */}
      <div className="tab-pills mt-16" style={{ marginBottom: 0 }}>
  ...
  // Footer content
  const footer = (
    <div className="postural-detail-actions" style={{ display: 'flex', flexDirection: 'column', width: '100%', gap: '8px' }}>
      {isConfiguring ? (
        <div className="configuration-panel card w-full" style={{ padding: 12 }}>
          <span className="section-label">CONFIGURE CORRECTIVE PLAN</span>
          
          <div className="config-row mt-8">
          ...
          <div className="sheet-actions mt-12" style={{ display: 'flex', gap: '8px' }}>
        ...
        <>
          {(!isTracking && !isResolved) && (
            <button className="btn-primary w-full" onClick={() => setIsConfiguring(true)} id="track-issue-btn">
              TRACK CORRECTIVE PLAN
            </button>
          )}
          {isTracking && (
            <button className="btn-ghost w-full" onClick={handleStopTracking} style={{ color: 'var(--accent-red)' }} id="stop-tracking-btn">
              STOP TRACKING PLAN
            </button>
          )}
  ...
  return (
    <BottomSheet 
      ...
      header={header} 
      footer={footer}
      className="postural-detail-sheet"
    >
      {/* DIAGNOSE TAB */}
      {activeTab === 'diagnose' && (
